...
function App() {
  ...
  const filterByAge = (minAge) => {
    const studentAge = students.filter((student) => student.age >= minAge);
    setFilteredStudents(studentAge);
  };

  // TODO: filterByGrade 함수를 작성하세요. 이 함수는 특정 학점을 매개변수로 받아 해당 학점의 학생들로 필터링해야 합니다.
  const filterByGrade = (grade) => {
    const studentGreade = students.filter((student) => student.grade === grade);
    setFilteredStudents(studentGreade);
  };
